...
def deduplicate_frames(input_dir, output_dir, threshold=20, frame_interval=30):
    ...
    Args:
        ...
        frame_interval: Seconds between each extracted frame (for timestamp calc)
    """
    ...
    for i, frame_path in enumerate(kept, 1):
        ...
        timestamp_sec = (frame_num - 1) * frame_interval
...
def main():
    ...
    parser.add_argument(
        ...
        help="Seconds between each extracted frame (default: 30)"
    )
    args = parser.parse_args()

    deduplicate_frames(args.input, args.output, args.threshold, args.frame_interval)
