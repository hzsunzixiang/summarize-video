...
# YouTube 视频（需要 COOKIES_BROWSER 认证）
make URL="https://www.youtube.com/watch?v=487AjvFW1lk" all
...
# 大模型生成翻译和演示稿后，用 make 编译所有输出
cd publish_tex/ && make       # 自动检测并编译所有存在的 .tex 文件
cd publish_md/ && make        # 自动检测并编译所有存在的 .md 文件
...
| `COOKIES_BROWSER` | （空） | 浏览器 cookies 来源：`chrome` / `firefox` / `safari` 等（YouTube 下载需要） |
...
**编译**（大模型生成文件后，始终用子目录 `make` 编译，不要直接调用 `xelatex` 或 `python3`）：
```bash
# tex 模式 — make 会自动检测 transcript_zh.tex 是否存在并编译
cd publish_tex/ && make

# md 模式 — make 会自动检测 transcript_zh.md 是否存在并编译
cd publish_md/ && make
```
...
make URL="https://www.youtube.com/watch?v=487AjvFW1lk" \
...
# 第二阶段：大模型辅助步骤
# 大模型读取 publish_*/transcript.* → 生成 transcript_zh.* 和 slides.*
# 然后用 make 编译所有输出：
cd publish_tex/ && make       # 自动编译 transcript.pdf + transcript_zh.pdf + slides.pdf
cd publish_md/ && make        # 自动编译 transcript.pdf + transcript_zh.pdf + slides.pptx
```
...
│   └── images/                   # 幻灯片图片
│
├── publish_md/                   # Markdown 模式输出（注意：不含 Python 脚本）
│   ├── Makefile                  # Python 转换系统（通过 ../md2pdf.py 引用上级目录脚本）
...
| YouTube 下载需要认证 | 在 `config.env` 中设置 `COOKIES_BROWSER = chrome`（或 firefox/safari） |
| 视频下载失败 | `brew upgrade yt-dlp`；确认 `COOKIES_BROWSER` 配置正确 |
