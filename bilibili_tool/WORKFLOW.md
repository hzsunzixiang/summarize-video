...
| `python-pptx` | `pip3 install python-pptx` | 生成 PPTX 演示文稿（可选） |
...
| 值 | 行为 |
|------|------|
| `yes` | 始终翻译为中文（大模型执行第 8 步） |
| `no` | 跳过中文翻译 |
| `auto` | 由大模型根据 `WHISPER_LANG` 决定：源语言非中文时翻译，源语言为中文时跳过 |
...
**执行者**：大模型（AI Agent / Skill），非脚本

**是否执行**：由 `TRANSLATE_ZH` 配置控制（`yes` / `no` / `auto`）
...
- [ ] 支持自动检测视频语言
- [ ] 支持 SRT/VTT 字幕输出格式
- [x] 大模型辅助：英文逐字稿 → 中文翻译（第 8 步）
- [x] 大模型辅助：中文翻译 → Beamer/PPTX 演示文稿（第 9 步）
- [x] 双发布模式：LaTeX（publish_tex/）和 Markdown（publish_md/）
- [x] 自动检测 xelatex 可用性，智能选择发布模式
- [x] TRANSLATE_ZH 配置项，控制中文翻译行为
