...
有一篇论文叫做 \textbf{AgentFold}，它训练模型使用一个叫做 \textbf{Fold（折叠）} 的压缩工具。这个工具接收两个输入：
...
\textbf{MCP-Zero} 这篇论文的核心想法：让语言模型用 AI 动态决定自己需要什么工具。
...
更复杂的做法是让 Context 经过三个语言模型：\textbf{Generator $\rightarrow$ Reflector $\rightarrow$ Curator}。
